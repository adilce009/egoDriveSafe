...
from sampling.agent_sampling import compute_agent_velocity, get_agent_context,get_relative_poses #compute_agent_velocity_modified
...
def generate_agent_sample_vectorized(
    ...
    (
        ...
        history_tl_faces,
        future_tl_faces,
        #history_agent_slice, # ADDED for original agent index
        #future_agent_slice  # ADDED for original agent index
    ...
    # we find the orientation of the target agent in world and later agent coordinate system that will give the
    # basis vectors. these basis vectors corresponds to the orientation of the target agent.
    # when a transformation (e.g. done inside get_related_pose function of agent_sampling.py) of a neighboring agent
    # location vector is done with respect to the agent_from_world matrix we get a distance of the neighboring agent
    ...
    future_coords_offset, future_yaws_offset, future_extents, future_availability = get_relative_poses(
        future_num_frames, future_frames, selected_track_id, future_agents, agent_from_world, agent_yaw_rad           # ADDED original_agent_indices
    )
    ...
    # print('future positions_m:', future_coords_offset)
    # print('history positions:', history_coords_offset)

    history_vels_mps, future_vels_mps = compute_agent_velocity(history_coords_offset, future_coords_offset, step_time)
    # print('future velocity', future_vels_mps)
    # print('history velocity:', future_vels_mps)

    history_velocity_x = history_vels_mps[:,0].reshape(-1,1)
    history_velocity_y = history_vels_mps[:,1].reshape(-1,1)
    ...
    frame_info = {
        ...
        #"target_agent_indices_original" : future_target_agent_indices,   #ADDED
        ...
        #"history_target_agent_indices_original" : history_target_agent_indices,
        "centroid": agent_centroid_m,
        "yaw": agent_yaw_rad,
        "speed": np.linalg.norm(history_vels_mps[0]),
        "history_velocity_x": history_velocity_x,
        'history_velocity_y': history_velocity_y,
        "scene_frame_index":state_index
    }
    ...
    vectorized_features = vectorizer.vectorize(selected_track_id, agent_centroid_m, agent_yaw_rad, agent_from_world,
                                               ...
                                               history_yaws_offset, history_velocity_x,history_velocity_y,history_availability, future_frames, future_agents)
