...
class Vectorizer:
    ...
    def _vectorize_agents(self, selected_track_id: Optional[int], agent_centroid_m: np.ndarray,
        ...
        agents_distances = agents_points[:,:len(agents_velocities[0]), 0]   # only considering distance along x axis; could find L2 distance
        agents_distances = np.expand_dims(agents_distances, -1)
        other_agents_polyline = np.concatenate([agents_velocities, agents_distances], axis=-1)
        ...
        agent_dict = {
            ...
            "other_agents_polyline": other_agents_polyline, # includes vx, vy and distance from ego
    ...
    def _vectorize_map(self, agent_centroid_m: np.ndarray, agent_from_world: np.ndarray,
        ...
        distances = []
        cw_mid_points = np.zeros((MAX_CROSSWALKS, 2), dtype=np.float32)
        ...
        for i, xw_idx in enumerate(crosswalks_indices[:MAX_CROSSWALKS]):
            ...
            cw_mid_points[i] = mid_point
            # distance to the midpoint
            dist_to_mid = np.linalg.norm(np.array(mid_point), axis=0) # check for result's accuracy; may be not required because mid_point is transformed and already expresses distance from agent
        ...
        #print('dist to crosswalks:', distance_to_crosswalks)
        ...
        ################## ######################
        # #####find up coming crosswalk
        # distance_to_crosswalk from the target agent is not helpful becuase crosswalk can be anywhere on different road branches

        # the idea here is take the current lane and lanes ahead points and find distances to all the crosswalks that are located
        # in front of the target agent. if the distance to the lane + lanes ahead points are below a threshold distance, the crosswalk must be
        # on the path of the target agent
        # lane = self.mapAPI.get_lane_as_interpolation(agent_lane_id, STEP_INTERPOLATION, INTERP_METHOD)
        # relative_coord_lane_points = transform_points(lane["xyz_midlane"][:, :2], agent_from_world)
        # relative_coord_lane_points = relative_coord_lane_points[relative_coord_lane_points[:, 0] > 0]
        # lanes_ahead = self.mapAPI.__getitem__(agent_lane_id).element.lane.lanes_ahead
        # concat_points = relative_coord_lane_points
        # if len(lanes_ahead) != 0:
        #     concat_points = np.zeros([len(lanes_ahead), MAX_POINTS_LANES * 2, 2])
        #     for i in range(len(lanes_ahead)):
        #         l_ah = self.mapAPI.get_lane_as_interpolation(lanes_ahead[i].id, STEP_INTERPOLATION, INTERP_METHOD)
        #         l_ah_points = transform_points(l_ah["xyz_midlane"][:, :2], agent_from_world)
        #         concated_points = np.vstack(
        #             (relative_coord_lane_points, l_ah_points))  # stack ta_lane and lane_ahead points
        #         concated_points = np.expand_dims(concated_points, axis=0)
        #         concat_points[i, :len(concated_points[i,:])] = concated_points
        # # all crosswalks mid points
        # cw_mid_points = cw_mid_points[cw_mid_points[:,0]>0]
        # if len(lanes_ahead) == 0:
        #     dist_to_crosswalk = self.find_upcoming_crosswalk(concat_points, cw_mid_points)
        # else:
        #     dist_to_crosswalk = np.array(len(concat_points))    # len(concat_points) = number of lanes ahead. we will have separate point streams for each of the lane ahead
        #     for i in range(len(concat_points)):
        #         dist_to_crosswalk = self.find_upcoming_crosswalk(concat_points[i], cw_mid_points)
        # nearest_crosswalk = min(dist_to_crosswalk)
        #if nearest_crosswalk <=3
        ...
        lanes_ahead = self.mapAPI.__getitem__(agent_lane_id).element.lane.lanes_ahead # lanes ahead of target agent lane
        lanes_ahead_tl_status = np.zeros((len(lanes_ahead)))
        for i in range(len(lanes_ahead)):
            lane_ahead_id = lanes_ahead[i].id
            lanes_ahead_tl_status[i] = self.mapAPI.get_tl_feature_for_lane(lane_ahead_id, active_tl_face_to_color)
        #lanes_ahead_tl_status
        ...
        return {
            ...
            "ta_lanes_ahead_tl_status" : lanes_ahead_tl_status,
    ...
    def find_upcoming_crosswalk(self, points, crosswalk_mids):
        # find the minimum distance from among the lane points
        points = points[points[:,0]>0]
        num_crosswalks = len(crosswalk_mids)
        dist = np.zeros([num_crosswalks])
        for i in range (num_crosswalks):
            d = np.linalg.norm(points - crosswalk_mids[i], axis=-1)
            dist[i] = min(d)
        return dist
